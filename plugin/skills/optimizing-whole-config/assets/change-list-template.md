...
| 委派spoke | 策略组（select/url-test/fallback/load-balance/relay） | `→ synthesizing-proxy-groups`（`create_proxy_group` · `update_proxy_group` · `delete_proxy_group`） | 跨链交回 spoke |
...
| 4 | 委派spoke | 策略组 `OpenAI` 的 `filter` | 裸 `us` 会顺带吃进 R**us**sia，应锚定 `\bUS\b` 或国旗 emoji | `→ synthesizing-proxy-groups`（`update_proxy_group`） | ☐ 待发起 |
