...
`list_local_nodes` 列本地源（`kind=local`）节点，**仅不透明 `node` 句柄（nd-）+ 非可逆序数 `display` + 白名单 `type` + `referencedBy`，凭证与原始节点名/来源名一律不出**；每个节点带 `referencedBy`（`via` 为有界类别序数）：

```jsonc
{ "ref": "ref-…", "count": N,
  "nodes": [ { "node": "nd-…", "display": "节点 1", "type": "ss",
              "referencedBy": [ { "kind": "chain-backend", "via": "引用 1" } ] } ] }
```
...
场景：用户想把本地源里的某个节点改名（目标名由用户给出，如 `香港-高级线路`）。

1. **拿 id、确认形态** — `list_node_sources` 找到该本地源，确认 `kind=local`（远程源不能直接改名，须改用 `rename-regex` 算子）。
2. **探测影响** — `list_local_nodes({ref})`，读目标节点 `node` 句柄对应的 `referencedBy`，假设得到：
   ```json
   [
     { "kind": "chain-backend", "via": "引用 1" },
     { "kind": "proxy-group-member", "via": "引用 2" },
     { "kind": "rule-policy", "via": "引用 3" }
   ]
   ```
3. **先预警，别闷头改** — 明确告诉用户：改名会断这 3 处引用，**其中链式后端引用不一并改会让整份配置在 mihomo 加载失败**。
4. **提议一并改，取得同意** — 提出连带修复清单：
   - `rename_local_node({ref, node: <该节点句柄>, to:"香港-高级线路"})` — 改源内容、仅动 name、凭证原样保留；节点句柄来自 `list_local_nodes`，绝不传原始节点名。
   - `update_proxy_group(<链式组 id>, …)` — 后端 `proxies` 里把旧节点名换成新名（`chain-backend`；先用 `list_proxy_groups` 按组名拿 id）。
   - `update_proxy_group(<手选组 id>, …)` — 手选成员里旧名换新名（`proxy-group-member`）。
   - `update_rule(<该规则 id>, policy:"香港-高级线路")` — `list_rules` 找到引用了旧节点名的规则后改 policy。
