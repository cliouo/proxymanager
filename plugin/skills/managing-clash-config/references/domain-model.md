...
`filter-regex`、`filter-useless`、`rename-regex`、`rename-template`、`flag-emoji`、`filter-type`、`sort`、`set-prop`、`dedup`、`filter-region`（共 10 种）。其中 `rename-template` 是**命名专用**的确定性模板命名（占位符 DSL，执行不调用模型；**每个管线最多一个**，旧版成分/预设行只读投影为等价模板，写接口拒绝双配置）。管理：`list_node_sources` 查 → `preview_node_operators` 试算(改正则前必做) → `add_/update_/delete_operator` / `reorder_operators`；命名专项走「智能命名」循环（`list_naming_targets` … `preview_naming_target` → `save_naming_plan`）。
