...
`list_local_nodes` 列本地源（`kind=local`）节点，**仅不透明节点句柄（`node`，nd- 开头）+ 有界脱敏显示名（`display`，即原始节点名经结构化脱敏后的可见文本）+ 白名单协议类型（`type`，未知统一 `unknown`），凭证与稳定键一律不出**；每个节点带 `referencedBy`（名称有界脱敏）：

```jsonc
{ "ref": "ref-…", "count": N,
  "nodes": [ { "node": "nd-…", "display": "HK-01", "type": "ss",
...
2. **探测影响** — `list_local_nodes({ref})`，读该节点 `node` 句柄对应的 `referencedBy`，假设得到：
   ```json
   [
   ...
   ```
3. **先预警，别闷头改** — 明确告诉用户：改名会断这 3 处引用，**其中 `网飞-链式` 是链式后端、不一并改会让整份配置在 mihomo 加载失败**。改名用 `rename_local_node` 的 `node` 句柄（绝不传原始节点名）。
