...
如果同一 profile 还含有 `name + type: direct + 可选 udp: true` 的冗余直连别名，且直连迁移预检
被这些非法筛选阻断、筛选修复又被旧 direct 字段阻断，就形成**跨资源恢复死锁**。每条候选仍须先
`preview_proxy_group_members`，随后改走 `preview_legacy_profile_repair` 获取 version + base ETag，最后用
`repair_legacy_profile` 把直连别名迁移和 2–16 个筛选修复放进一个完整候选、一张确认卡和一次原子提交。
该工具会继续拒绝已合法筛选的普通批量编辑、非安全 direct 形态、遗漏的其它非法组和任何未通过完整渲染的候选。

若直连 + 非法筛选之外还被订阅兼容问题或陈旧链后端阻塞，改走
`preview_legacy_chain_profile_repair` → `repair_legacy_chain_profile`。它只能隔离完整的 VLESS `spx` 原始行，
或删除已证明无后端且没有隐藏引用的单链 / 前置池组合；不会放宽最终链校验，也不会静默丢弃 `spx`。
...
`preview_legacy_profile_repair` · `repair_legacy_profile`
· `preview_legacy_chain_profile_repair` · `repair_legacy_chain_profile`
