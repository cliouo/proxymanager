...
- [kind × 字段矩阵](#kind-字段矩阵)
- [谁渲染时算](#谁渲染时算)
- [type 语义：select/url-test/fallback/load-balance/relay](#type-语义)
- [每种 kind 一个最小示例](#最小示例)
...
- **`type` = mihomo 原生选路行为**。`select/url-test/fallback/load-balance/relay`，逐字下发给内核。
...
- `exclude-type`：按节点 type 排除，逗号分隔如 `"Direct,Reject"`（**非正则**，schema L70）。
...
- `single-sub` + `bound_subscription_id` → `proxies` = 该订阅源本次渲染的存活节点名（`nodesBySub.get(sub.name)`）。绑定源不存在 / 无存活节点 → 告警并保留原字段，不静默丢（L416-428）。
- `collection-scope` 遗留 + `bound_collection_id` → `proxies` = 成员订阅各自存活节点，按成员顺序 + 源内顺序去重（L436-450）。
- `include-all-proxies` / `include-all` 组：渲染时**自动追加一段锚定 `exclude-filter`**，剔除链式代理的克隆节点（`realizeChainWraps` 注入 `proxies:` 的 chain-wrap 克隆，避免「全部节点」把链式克隆也吸进来，也防止智能前置池经链路自我回环），与用户已有 `exclude-filter` 用 `|` 合并（L714-728）。
...
> 验证正则用 `preview_proxy_group_members`：它在 ProxyManager 侧按 mihomo 语义（filter 命中保留、再去掉 exclude-filter 命中）对真实节点名试算，**改 `filter`/`exclude-filter` 前后都应调用**。常见坑：裸 `us` 会顺带吃进 A-`us`-tralia / R-`us`-sia，应改用单词边界 `\bUS\b` 或国旗 emoji 锚定。

<a id="type-语义"></a>
## type 语义：select/url-test/fallback/load-balance/relay
...
| `relay` | 链式中继，流量依次经过 `proxies` 列出的各跳 | 否 | — |

健康检查类型 = `HEALTH_CHECK_TYPES`（schema L42）：`url-test` / `fallback` / `load-balance`。仅这三种接受健康检查字段：`url` / `interval` / `lazy` / `expected-status` / `max-failed-times` / `timeout`（外加 url-test 的 `tolerance`）。`select` / `relay` 不做健康检查。
...
- `update` 校验（L232-234）：至少改一个字段；可空字段传 `null` 表示**清除**该键（`filter`/`exclude_filter`/`exclude_type`/`dialer_proxy`/`section`/`notes`，L249）。
