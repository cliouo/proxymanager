...
| PC-015 | P1       | VLESS                                                   | packet encoding                                     | `packetEncoding=none` from a target-specific V2Ray URI export was rejected even though the same node's Clash and sing-box exports omit the field and both target runtimes select XUDP by default                                                                  | Omitted, explicit `none`, and explicit XUDP map to canonical XUDP; `packet`/`packetaddr` map canonically to packetaddr; empty and unknown values still reject                                            | Fixed Mihomo converter/runtime source plus a 2026-08-03 credential-redacted target-differential export of one identical VLESS node                                                              | Normalize exact URI `none` to explicit `packet-encoding: xudp`; keep the closed enum, alias-collision checks, and explicit-empty/unknown rejection                                                               | Parser and full subscription-normalisation tests for omitted/xudp/packet/packetaddr/none/empty/unknown                                                                                                           |
...
## PC-015: target-compatible VLESS `packetEncoding=none` was rejected

Production reproduction on 2026-08-03 compared credential-redacted Clash,
sing-box, V2Ray, and plain-URI exports for one identical VLESS node. Clash and
sing-box omitted packet encoding, while the Base64-decoded V2Ray export was
byte-identical to the plain URI and carried `packetEncoding=none`. ProxyManager
accepted the Clash output but rejected the URI. Mihomo's share converter accepts
that spelling and omits the field; its outbound constructor then selects XUDP,
and sing-box likewise selects XUDP when its field is omitted. The adopted Mihomo
target contract is therefore omission/explicit `none`/explicit XUDP -> canonical
XUDP and `packet`/`packetaddr` -> canonical packetaddr. Explicit empty and unknown
values still reject. Residual risk: this is a target-specific compatibility
mapping for an ecosystem URI extension and must be rechecked on target upgrades.
