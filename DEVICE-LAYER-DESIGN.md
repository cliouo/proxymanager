...
> 状态：设计定稿待批准。覆盖四条线：信息架构（IA）、设备层（P1 补丁 / P2 设备级功能）、
> 模版类型（T）。每条线独立可发布。事实依据全部来自当前代码（文中给出 文件:行 号），
> 不依赖记忆。
...
| 文件 | 改动 |
|---|---|
| `components/nav.ts` | 新增 `EXTENSIONS_NAV`（链式代理、Tailscale、扩展中心）；PROFILE_NAV 移除这两项。`ALL_NAV` 仍含全部 → `titleForPath`/Topbar 零改动 |
| `components/Sidebar.tsx` | 删 `PROMOTED_SCENARIOS`、`SCENARIO_LABEL_OVERRIDES`、`/api/v1/scenarios` fetch、「更多场景」条件块；渲染静态「扩展」组 |
...
P1 设备 CRUD 不走 `/api/v1/ops`。P2 设备级功能的 op 走现有 dispatcher，
OpContext（`dispatch.ts:87-94`）增 `device?: DeviceStore`，deviceId 由 payload
信封携带、route 层解析校验（profile 作用域仍按 `profileScope.ts:42-61` 的
query → cookie → default 优先级）。
...
### 7.1 设备入口 —— 绑定与设置页（`profiles/[id]/page.tsx`）

在现有四个 panel（基本信息 / 节点来源 / 订阅链接 / 危险区，page.tsx:289-558）之间
插入第五个 panel「设备」（位于订阅链接之后）：

```
┌ 设备 ────────────────────────────────────────────┐
│ 共享配置服务所有设备；每台设备可叠加少量差异。       │
│                                                  │
│ ▪ macbook        2 项差异   [链接] [设置] [删除]   │
│ ▪ home-server    4 项差异   [链接] [设置] [删除]   │
│ ▪ iphone         1 项差异   [链接] [设置] [删除]   │
│                                                  │
│ [＋ 添加设备]                                     │
└──────────────────────────────────────────────────┘
```

「链接」打开 `DistributeDrawer`（`components/DistributeDrawer.tsx`），抽屉扩展为
按设备列出订阅 URL（顶部保留 profile 共享链接）。

### 7.2 设备详情页（新路由 `profiles/[id]/devices/[deviceId]/page.tsx`）

```
┌ Topbar: 设备 · home-server            [订阅链接 ⧉] ┐
│                                                    │
│ ① 差异清单（结构化卡片，一键移除=回退共享值）          │
...
│                                                    │
│ ③ 生效预览（只读 diff：共享渲染 vs 本设备渲染）        │
│   ← GET .../preview，CodeMirror merge 视图          │
│                                                    │
│ ④ [P2] 本设备的功能：Tailscale 卡片（三态）           │
│                                                    │
│ ⑤ 危险区：删除设备（确认语句明示订阅链接将 404）        │
└────────────────────────────────────────────────────┘
```
...
## 9. P2：设备级功能（设计到合同级，实现细节随 P1 落地后定）
...
设备实体增 `features: Record<scenarioId, unknown>`（该功能的私有实例数据）。
Tailscale 实例：`{ hostname, authKey, controlUrl?, stateDir?, exitNode?, extraCidrs? }`。

渲染注入点在 §4.1 管线第三步：功能声明一个纯函数
`emitForDevice(instance) → { proxies[], groups[], rules[] }`，产物追加进 patch 后的
文档，再走 `validateFinalRenderedConfig`（组成员引用同批注入的节点，DAG/策略目标
校验天然覆盖悬空引用）。**与现状的关键区别**：tailscale 产物不再写进共享 base
（现状是 base 字面 proxies + 组 + 规则，`tailscale/scenario.ts:1-32`），改为渲染期
按设备生成 —— 共享层 /base 里从此看不到 tailscale 节点，避免「一套 hostname 全设备
共用」的根因。

### 9.3 约束与校验

- hostname 唯一性：**跨全部 profile 的全部设备**查重（同一 tailnet 一个 hostname
  一台机器）—— 写入 preflight 时全表扫（数据量 ≤ 16×profile 数，直查无索引）；
- auth-key 审计掩码沿用 hasAuthKey 先例；
- ops 走 dispatcher，OpContext 增 device 作用域（§6.3）。

### 9.4 迁移与页面

- `migrate:tailscale-device`：把存量 base 里的 `type: tailscale` 节点 + 关联组/规则
  迁为「某设备的 tailscale 实例」（交互式选目标设备或新建，dry-run + `--apply`；
  迁移后从共享 base 删除原件）；
- `/scenarios/tailscale` 页转型为**总览页**：设备 × 接入状态矩阵，每行「去配置」跳
  设备详情页的 Tailscale 卡片 —— 侧边栏「扩展 → Tailscale」入口保持有效，不留死链。
