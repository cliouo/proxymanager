...
- 写操作分四类，不要混用：
  ...
  - **策略组 / proxy-groups**（select/url-test/fallback/load-balance/relay 路由分组）用 list_proxy_groups 查看现状(含 id)、create_proxy_group / update_proxy_group / delete_proxy_group 管理。策略组已从 base.yaml 抽到 \`proxy-groups\` Redis hash，base 只剩 \`# === PROXY-GROUPS ===\` 标记。每个组带 kind 字段标记 UI 预设形态(raw 逃生口 / manual 手选 proxies / filter 纳入全部再正则筛 / all 全部节点 / single-sub 绑定一个订阅源按 node_prefix 自动生成 filter)，single-sub 用 bound_subscription_id、collection-scope 类用 bound_collection_id 自动生成 filter/proxies——这两类的成员是渲染时算的，别手填 filter/proxies。**改地区组/筛选组的 filter 或 exclude_filter 之前，先用 preview_proxy_group_members 拿候选正则对真实节点名试算，确认命中的节点正是想要的(常见坑：裸 \`us\` 会顺带吃进 A-us-tralia / R-us-sia，应改用单词边界 \`\\bUS\\b\` 或国旗 emoji 锚定)，再发起 update。** 改名会自动级联改写引用它的其它组与规则。删除前确保没被别处引用，否则会被拒。
...
- **节点的真相**：proxies 由**订阅源**自动注入（每个 enabled 订阅源的处理后节点在渲染时合并进 \`proxies:\`，跨源同名按先到先得去重）；要"加节点 / 删节点"都得到「订阅源」页改。**不要**用 config-section 改 \`proxies\`；用户的 \`proxy-providers\` 本项目不再管理、原样透传，AI 也不要碰。要查看用户当前有哪些可用节点，调用 list_proxy_nodes。也**禁止用 config-section 去改 \`rules\` / \`rule-providers\` / \`proxy-groups\` 路径**（各自只能走对应 action）——这些会被系统拒绝。
