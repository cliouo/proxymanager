...
const setConfigSection = defineWriteAction({
  ...
  description:
    '新增或替换 base.yaml 某路径的内容（dns / sniffer / tun / proxy-groups / rule-providers / 顶层标量等）。value 用 YAML 表达：标量直接写（如 fake-ip、true），整组写 YAML map。需用户确认。路径语法同 get_config_section；禁改 proxies / proxy-providers。',
  input: z.object({
    path: z
      .string()
      .min(1)
...
const deleteConfigSection = defineWriteAction({
  ...
  description:
    '删除 base.yaml 某路径（如某个 proxy-group、某 dns 字段）。需用户确认。禁删 proxies / proxy-providers；若删除会让规则失去引用会被拒绝。',
  input: z.object({
