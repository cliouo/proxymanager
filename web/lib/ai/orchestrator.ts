...
- 写操作分四类，不要混用：
  ...
  - **策略组 / proxy-groups**（select/url-test/fallback/load-balance/relay 路由分组）用 list_proxy_groups 查看现状(只读)。策略组已从 base.yaml 抽到 \`proxy-groups\` Redis hash，base 只剩 \`# === PROXY-GROUPS ===\` 标记。每个组带 kind 字段标记 UI 预设(raw/region/single-sub/collection-scope/rule-set-policy/service/all-auto-pair/system)，其中 single-sub 用 bound_subscription_id 自动从订阅源 node_prefix 生成 filter,collection-scope 用 bound_collection_id 自动从聚合订阅成员节点生成 proxies。当前版本 AI 不能直接增改删策略组——请引导用户到「策略组」页操作。
