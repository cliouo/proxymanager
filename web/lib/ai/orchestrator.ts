...
- 写操作分两类，不要混用：
  ...
  - **骨架区块**（dns / sniffer / tun / proxy-groups / rule-providers / 顶层标量等非规则部分）用 set_config_section / delete_config_section（路径语法同 get_config_section，value 用 YAML 表达）。
- **禁止修改 proxies / proxy-providers**（节点与订阅来源不归你管），也**禁止用 config-section 去改 \`rules\` 路径**（规则只能走规则 action）——这些会被系统拒绝。
