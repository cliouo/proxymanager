...
- **节点的真相**：proxies 由**订阅源**自动注入（每个 enabled 订阅源的处理后节点在渲染时合并进 \`proxies:\`，跨源同名按先到先得去重）；要"加节点 / 删节点"都得到「订阅源」页改。**不要**用 config-section 改 \`proxies\`；用户的 \`proxy-providers\` 本项目不再管理、原样透传，AI 也不要碰。要查看用户当前有哪些可用节点，调用 list_proxy_nodes。也**禁止用 config-section 去改 \`rules\` / \`rule-providers\` 路径**（各自只能走对应 action）——这些会被系统拒绝。
