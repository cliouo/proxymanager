...
- 2026-05-20：M3-B 历史 & 撤销三阶段落地。**B1（`extension-v0.1.2`）**：RecentWrite 每条加 Undo 按钮——DELETE `/api/v1/rules/{ruleId}` + 可选 auto-reload，本地标 `undone:{ts,reloaded,error?}` 持久化，失败 inline 提示。**B2（web）**：Upstash 加 `audit:events` ZSET（score=ts，member=event id）+ `audit:by_id` Hash 存 JSON payload，pipelined 写，cap 1000 opportunistic 修剪。`rules POST/PUT/PATCH/DELETE` 入口插 `recordEvent()`（携带 before/after 快照），`X-Source` header → `resolveActor()` 区分 actor。新端点 `GET /api/v1/history` 时间倒序分页（`before_ts`）、`POST /api/v1/history/{id}/undo` 通用反向操作（带 `updated_at` 乐观并发校验，409 拒绝冲突）。**B3（web）**：`/history` 页时间表 + Undo CTA，inverse 事件回填到列表顶，sidebar 加导航。**`extension-v0.1.3`**：所有 backend 请求带 `X-Source: extension` header。45 测试仍通过
- 2026-05-20：M3-C 批量操作（`extension-v0.1.4`）——补齐 M3-B 留下的 `rules/batch` 审计盲点 + 扩展端 popup 批量入口。**C1（web）**：batch 路由在 planning loop 中按 `existingMap` 拿到 before 快照，commit 后串行 emit `recordEvent`，覆盖 create/update/delete 三种 op；45 测试仍通过。**C2 + C3（extension）**：每张 ResultCard 通过 `onSubmissionChange(cardId, submission|null)` 把"如果点 Write 会发什么"告诉父组件 popup，popup 用 cardId-keyed `Map` 聚合；当 ≥2 张卡 ready 时顶部出现 accent-边框 "Batch write" 卡，点击：单次 `createRulesBatch` RPC → 单次 reload → 把 N 条 fresh 条目按原顺序 prepend 到 recent-writes → 写成功的卡从结果区消失，失败的留下显示部分进度。后端新 `backendCreateRulesBatch` + `BatchCreateOutcome` 契约
- 2026-05-21：多源订阅 + Collection + 渲染时 inline + chained-proxy 改造（5 phase 改建）。**P1+P2 (`a2165d7`)**：Subscription schema 加 `kind: remote|local` / `ttl_ms`（默认 10min）/ `content`（local 用）/ `custom_headers` / `tags` / `last_error`，老数据通过 safeParse + defaults 在线迁移。Sub-Store 风格 fetch cache (`fetch-cache:{sha16(url+ua+headers)}` 独立 Redis key + EX TTL 自动过期)，新 `resolveSubscriptionContent(sub, opts)` 单入口分发本地/远程 + cache hit/miss + `?noCache=1` 强刷。**P3 (`f42236f`)**：Collection 模型（`subscription_ids` 显式 + `subscription_tags` 自动 + `dedup_by` + `name_prefix`），CRUD endpoints + `/collections` 页 + sidebar 链接 + `/subscriptions` 页大改加 kind 切换 / 本地 YAML textarea / TTL / tags 输入。**P4 (`4f2d4ac`)**：新 `lib/engine/collectionExpander.ts` — 读 base.yaml 顶层 `pm-inline-collections: [...]` 字段，解析每个 collection → 拉所有子 sub（走 cache）→ 合并 proxies + dedup + 加前缀 + 检测跨集合/与 base 冲突 → append 进 `proxies:` 段 → 删除 `pm-inline-collections` 字段。`/api/sub/{token}/{profile}` 和 `/api/v1/base/parsed` 都过这个 pipeline，scenario UI 看到的节点跟 Mihomo 看到的一致。**P5 (`...`)**：`chained-proxy` 重构成 wrapping-group 模型——Fixed chain = 一个 `{name, type:select, proxies:[B], dialer-proxy:F}` group；Pool chain = 两个 group（fronts pool + wrap）。引用名在 render 时解析，所以可以链接 collection 节点。Ops 维持原名字（`set-fixed-chain` 等），payload 改成基于 `chainName`，inverse 全部重写，环路检测改为走 proxy-group 边。45 测试仍通过
- 2026-05-21：`chained-proxy` 场景落地（链式代理，第一个真正动 base.yaml 的场景）——验证 `BaseStore.withDocument` 在真实场景下的 round-trip 安全。用户两种链法都支持：(1) **Fixed chain**：B 节点上设 `dialer-proxy: F`，路径 client→F→B→exit，一对一硬编码；(2) **Pool chain**：建一个 `select` 类型 proxy-group `poolName` 含成员 [F1..Fn]，B 节点设 `dialer-proxy: poolName`，Clash UI 改 pool 选中项即换前端无需改 YAML。底层全部用 Mihomo `dialer-proxy` 字段表达，不引入 `relay` group 类型也不复制 B。新增 `lib/engine/structured.ts` 抽 proxies + proxy-groups 结构化信息 + `GET /api/v1/base/parsed` 单点读端点（所有 base.yaml 场景未来共用）。场景 5 个 op（set-fixed-chain / clear-chain / create-pool-chain / update-pool-members / delete-pool-chain）+ 全部对应 inverses；每个 op 携带 before/after 快照让 `/history` 通过 InverseHandler 撤销；环路检测走 dialer-proxy 边集 DFS 拒绝自指。pool 创建时打 taxonomy `kind:'custom'` 标签防止后续 regional/platform 场景误列。UI `/scenarios/chained-proxy` 分两段（Fixed chains / Pool chains），每行可编辑/删除，Add CTA 用 picker 从 `/api/v1/base/parsed` 拿候选。45 测试仍通过
- 2026-05-21：场景框架（Phase 0 + Phase 1 迁移）——从「工具」升级到「场景插件平台」。用户决策：分类元数据走 Redis Hash 方案 A（`taxonomy:groups`）、mutation 单点 `POST /api/v1/ops`、先架子后场景、迁移 /rules 验证框架。**Phase 0 架构**：`lib/scenarios/_shared` 立四件套——`types.ts`（Scenario / OpContext / OpHandler / InverseHandler / AuditTarget / GroupTag 契约）、`baseMutator.ts`（BaseStore.withDocument 用 `yaml` 包的 Document API 保留注释 + key 顺序，提交走 ETag 并发守卫）、`GroupTaxonomy.ts`（TaxonomyStore over `taxonomy:groups` Hash，与 base.yaml 解耦）、`dispatch.ts`（单入口分发器：注册表查 handler → 注入 OpContext → 跑 → 把 `events` 串成 `${scenarioId}.${action}` 通过 recordEvent 落库）；新端点 `POST /api/v1/ops` 单点接所有 mutation + `GET /api/v1/scenarios` 给 sidebar 拉描述符；audit schema 把 `op` 从 enum 3 个值升级到 namespaced 字符串、加 `target: discriminatedUnion<rule|proxy|proxy-group|base>` 字段、`ruleId` 留作向后兼容。**Phase 0 UI**：Sidebar 分三段（Primary / Scenarios / Footer），Scenarios 子项 mount 时拉端点动态渲染；新增 `/scenarios` 索引页 + `/scenarios/dev-echo` 验证页可手动 ping/mark 看 dispatcher 闭环。**Phase 1 /rules 迁移**：新 `rule-anchor-append` 场景持有 5 个 op（create / replace / patch / delete / batch-create）+ 3 个 inverse；老的 `/api/v1/rules/*` REST 路由变成调 dispatch 的 thin shims（GET 查询不动、batch 留在原实现）；`/history/{id}/undo` 改成从注册表查 inverse handler，老的 `rule.*` 事件桥接到 `rule-anchor-append` 场景维持可撤销；`/rules` 页搬到 `/scenarios/rule-anchor-append`，老 URL 留 client-side redirect；扩展 API 表面零变化。45 测试仍通过
